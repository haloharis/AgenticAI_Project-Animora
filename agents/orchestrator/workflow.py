    ...
    async def run_pipeline(
        ...
        try:
            ...
            story = await asyncio.get_event_loop().run_in_executor(
                None, lambda: StoryAgent().run(user_prompt, style)
            )
            ps.story = story
            ...
            await self._emit(job_id, {"phase": "story", "status": "completed", "progress": 33})
        ...
        # --- Phase 2: Audio ---
        await self._emit(job_id, {"phase": "audio", "status": "running", "progress": 33})
        ...
        try:
            ...
            manifest = await asyncio.get_event_loop().run_in_executor(
                None, lambda: AudioAgent().run(ps.story, job_id)
            )
            ...
            await self._emit(job_id, {"phase": "audio", "status": "completed", "progress": 66})
        except Exception as e:
        ...
        # --- Phase 3: Video ---
        await self._emit(job_id, {"phase": "video", "status": "running", "progress": 66})
        ps = update_phase_status(ps, "video", PhaseStatus.running)
        try:
            from agents.video_agent.agent import VideoAgent
            video_path = await asyncio.get_event_loop().run_in_executor(
                None, lambda: VideoAgent().run(ps.story, ps.timing_manifest, job_id)
            )
    ...
    async def rerun_phase(self, job_id: str, phase: str) -> PipelineState:
        ...
        try:
            if phase == "story":
                from agents.story_agent.agent import StoryAgent
                story = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: StoryAgent().run(ps.user_prompt, ps.style)
                )
                ps.story = story
