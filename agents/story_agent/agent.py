...
class StoryAgent:
    ...
    def _build_prompts_node(self, state: StoryState) -> Dict[str, Any]:
        ...
        for scene in story_dict.get("scenes", []):
            if not scene.get("visual_prompt"):
                scene["visual_prompt"] = build_visual_prompt(scene, characters, style)
            print(f"\n{sep}")
    ...
    def run(self, user_prompt: str, style: str = "cinematic", log_fn=None, progress_fn=None) -> Story:
        self._log_fn = log_fn
